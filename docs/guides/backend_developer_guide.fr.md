...
La route d'administration `/maintenance/upload-gpx` permet de réimporter les attributs des caches :
...
- **Hashage** : Mots de passe avec argon2id via Passlib (bcrypt conservé uniquement pour vérifier les hashs existants ; les comptes migrent vers argon2id progressivement à la reconnexion)
...
| `GET` | `/api/geo/FR/adm1.geojson` | FeatureCollection des régions (StaticFiles) |
| `GET` | `/api/geo/FR/adm2.geojson` | FeatureCollection des départements (StaticFiles) |
