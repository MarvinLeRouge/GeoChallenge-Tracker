...
def teardown_cleanup(inserted_ids, initial_counts):
    ...
    if inserted_ids["caches"]:
        ...
        get_collection("challenges").delete_many(
            {"cache_id": {"$in": list(inserted_ids["caches"])}}
        )
...
def test_subset_only_creates_and_is_idempotent(initial_counts, inserted_ids):
    ...
    ids = [
        insert_cache_with_attrs(
            attrs=[{"attribute_doc_id": attr_doc_id, "is_positive": True}],
        ...
        insert_cache_with_attrs(
            attrs=[{"attribute_doc_id": ObjectId(), "is_positive": True}],
            inserted_ids=inserted_ids,
            title="NC",
        ),
    ]
...
def test_no_candidates_in_subset(inserted_ids):
    ...
    ids = [
        ...
        insert_cache_with_attrs(
            attrs=[{"attribute_doc_id": attr_doc_id, "is_positive": False}],
            inserted_ids=inserted_ids,
            title="NEG",
        ),
    ]
