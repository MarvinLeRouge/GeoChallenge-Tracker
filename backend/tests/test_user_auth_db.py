...
def test_verify_password_valid():
    ...
    assert user is not None, "L'utilisateur n'existe pas dans la base."
    assert verify_password(TEST_PASSWORD, user["password_hash"]) is True, "Le mot de passe fourni ne passe pas"
...
def test_verify_user_password_invalid():
    ...
    assert verify_password(TEST_PASSWORD + "a", user["password_hash"]) is False, "le mot de passe erroné fonctionne"
