...
class TestCalendarVerificationService:
    ...
    def mock_db(self):
        ...
        class MockCollection:
            ...
            async def find_one(self, query):
                if "cache_types" in str(self.__class__.__name__) or "name" in query and query.get("name") == "Traditional Cache":
                    return {"_id": ObjectId(), "name": "Traditional Cache"}
                elif "cache_sizes" in str(self.__class__.__name__) or "name" in query and query.get("name") == "Regular":
    ...
    async def test_verify_user_calendar_with_filters(self, mock_db):
        ...
        # Mock found caches data
        mock_found_caches = [
            {"found_date": datetime(2024, 1, 1), "cache_info": {"cache_type_id": ObjectId(), "cache_size_id": ObjectId()}},
        ]
        mock_db.found_caches.data = mock_found_caches
