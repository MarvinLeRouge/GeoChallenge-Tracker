...
def _build_param(points: List[Tuple[float, float]]) -> str:
    # API expects "lat,lon|lat,lon|..."
    return "|".join(f"{lat},{lon}" for (lat, lon) in points)

def _split_params_by_url_and_count(all_param: str) -> List[str]:
    """
    Split a large 'locations' parameter string into chunks that satisfy:
      - len(prefix + param) <= URL_MAXLEN (approx)
      - at most MAX_POINTS_PER_REQ locations per chunk
    Heuristic: iteratively take the largest prefix not exceeding URL_MAXLEN, then trim to 100 points if needed.
    """
...
async def fetch(points: List[Tuple[float, float]]) -> List[Optional[int]]:
    """
    Returns list of elevations aligned with input points. Never raises; returns None entries on failure.
    """
