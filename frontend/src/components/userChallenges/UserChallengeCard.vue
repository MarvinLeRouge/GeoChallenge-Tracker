...
<template>
    <div class="rounded-lg border shadow-sm p-3 transition" :class="zebra ? 'bg-gray-50' : 'bg-white'">
        <h3 class="font-semibold flex flex-wrap items-center gap-2">
            <CheckCircleIcon v-if="challenge.status === 'accepted'" class="w-5 h-5 text-green-600 shrink-0"
                aria-hidden="true" />
            <XCircleIcon v-else-if="challenge.status === 'dismissed'" class="w-5 h-5 text-red-600 shrink-0"
                aria-hidden="true" />
            <ClockIcon v-else-if="challenge.status === 'pending' && challenge.computed_status != 'completed'"
        ...
        <div class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-600">
            ...
            <a v-if="challenge.cache?.GC" :href="`https://www.geocaching.com/geocache/${challenge.cache.GC}`"
                target="_blank" rel="noopener" class="flex items-center gap-1 hover:text-blue-600 truncate"
        ...
        <div class="mt-2">
            <div v-if="challenge.progress && challenge.progress.percent !== null"
                class="w-full bg-gray-200 rounded h-2">
                <div class="bg-green-500 h-2 rounded" :style="{ width: challenge.progress.percent + '%' }" />
            </div>
        ...
        <div class="flex flex-wrap gap-2 mt-3">
            <!-- Détails -->
            <button class="p-2 rounded-full border bg-white hover:bg-gray-100" @click="$emit('details', challenge)"
                title="Détails">
            ...
            <button
                ...
                class="p-2 rounded-full border bg-white hover:bg-green-50" @click="$emit('accept', challenge)"
                title="Accepter">
            ...
            <button
                ...
                class="p-2 rounded-full border bg-white hover:bg-red-50" @click="$emit('dismiss', challenge)"
                title="Ignorer">
                <XMarkIcon class="w-5 h-5" />
            </button>

            <!-- Reset vers pending (si accepted ou dismissed) -->
            <button v-if="['accepted', 'dismissed'].includes(challenge.status)"
                class="p-2 rounded-full border bg-white hover:bg-amber-50" @click="$emit('reset', challenge)"
                title="Réinitialiser (pending)">
                <ArrowUturnLeftIcon class="w-5 h-5" />
            </button>

            <!-- Tasks -->
            <button v-if="['accepted', 'completed'].includes(challenge.status) || challenge.computed_status === 'completed'" class="p-2 rounded-full border bg-white hover:bg-gray-100" @click="$emit('tasks', challenge)"
                title="Tâches">
