...
import {
  ...
  type CrosshairPicker,
} from "./crosshairPicker";
...
const emit = defineEmits<{
  ...
  (
    e: "centerChanged",
    payload: { lat: number; lng: number; coords: string },
  ): void; // ← AJOUT ICI
}>();
...
function init() {
  ...
  L.tileLayer(tileUrl(), { maxZoom: 19, attribution: attribution.value }).addTo(
    map,
  );
...
function tryRecenterOnce() {
  ...
  const target =
    fromProp && Number.isFinite(fromProp[0]) && Number.isFinite(fromProp[1])
      ? fromProp
      : fromUser &&
          Number.isFinite(fromUser.lat) &&
          Number.isFinite(fromUser.lon)
