...
export function useMapPopup(options?: {
  ...
  const loadingHtml =
    options?.loadingHtml ??
    `<div class="p-2 text-sm text-gray-600">Chargement…</div>`;
  const errorHtml =
    options?.errorHtml ??
    `<div class="p-2 text-sm text-red-600">Erreur de chargement</div>`;
  ...
  async function openCachePopup(cacheId: string, marker: L.Marker) {
    // Popup “chargement”
    if (!marker.getPopup())
      marker.bindPopup(loadingHtml, { closeButton: true });
    ...
    try {
      ...
      if (!d) {
        const { data } = await api.get<CacheDetails>(
          `/caches/by-id/${cacheId}`,
          {
