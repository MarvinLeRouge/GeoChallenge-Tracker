...
export function useTargets() {
  ...
  async function fetchRefreshStatus(): Promise<TargetsRefreshStatus> {
    const { data } = await api.get<TargetsRefreshStatus>(
      "/my/targets/refresh-status",
    );
  ...
  async function evaluateAll(force = false): Promise<EvaluateAllResult> {
    ...
    try {
      const { data } = await api.post<EvaluateAllResult>(
  ...
  async function init(): Promise<void> {
    try {
      ...
      toast.error("Erreur lors du chargement des targets", {
        description: msg,
      });
    }
  }
