...
async function submit() {
  if (!canSubmit.value) {
    toast.error("Sécurité du mot de passe", {
  ...
  try {
    ...
    if (status === 422) {
      ...
      const items: FastAPIValidationItem[] = Array.isArray(detail)
        ? detail
        : [];
      const isPwd = items.some(
        (x) =>
          (Array.isArray(x.loc) && x.loc.includes("password")) ||
          /password|mot de passe/i.test(x.msg ?? ""),
      );
      ...
      error.value =
        typeof detail === "string" ? detail : "Inscription impossible.";
