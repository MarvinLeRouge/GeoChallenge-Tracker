        ...
        <div v-else>
          ...
          >
            ...
            <button
              ...
              :class="picking ? 'bg-indigo-100 border-indigo-400' : 'hover:bg-gray-50'"
            ...
            >
              <svg
                ...
                aria-hidden="true"
              >
                <path d="M11 2v3a1 1 0 002 0V2h-2zm0 17v3h2v-3a1 1 0 10-2 0zM2 11h3a1 1 0 100-2H2v2zm17 0h3v-2h-3a1 1 0 100 2z" />
                <circle
                  cx="12"
                  cy="12"
                  r="3"
          ...
          <p
            v-else-if="!nearbyMode && refreshStatus && !refreshStatus.needs_refresh && refreshStatus.last_targets_evaluated_at"
            class="text-xs text-gray-500 mt-1"
          >
          ...
          <!-- No import yet -->
          <p
            v-else-if="!nearbyMode && refreshStatus && !refreshStatus.last_not_found_import_at"
            class="text-xs text-gray-500 mt-1"
          >
...
const { targets, nbItems, loading, evaluating, refreshStatus, init, fetchTargetsNearby } =
  useTargets()
...
async function toggleNearbyMode() {
  ...
  if (loc?.lat != null && loc?.lon != null) {
    ...
    toast.info('Aucune position enregistrée — cliquez sur la carte pour définir un centre.')
    picking.value = true
  }
}
...
function plotTargets(items: TargetItem[]) {
  ...
  for (const t of items) {
    ...
    marker.bindPopup(renderTargetPopup(t), { closeButton: true, autoPan: true })
