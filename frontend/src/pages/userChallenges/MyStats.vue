<template>
  <div class="max-w-4xl mx-auto">
    <!-- Header -->
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-gray-900 mb-2">Mes statistiques</h1>
      <p class="text-gray-600">Vue d'ensemble de votre activité géocaching</p>
    </div>
    ...
    <div v-else-if="stats" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <p class="text-sm font-medium text-gray-500">Caches trouvées</p>
              <p class="text-2xl font-semibold text-gray-900">{{ stats.total_caches_found }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-semibold text-gray-900">{{ stats.total_challenges }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-semibold text-gray-900">{{ stats.active_challenges }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-semibold text-gray-900">{{ stats.completed_challenges }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div class="space-y-3">
            <div>
              ...
              <p class="text-sm text-gray-900">{{ formatDate(stats.created_at) }}</p>
            </div>
            <div v-if="stats.first_cache_found_at">
              <p class="text-sm font-medium text-gray-500">Première cache trouvée</p>
              <p class="text-sm text-gray-900">{{ formatDate(stats.first_cache_found_at) }}</p>
            </div>
            <div v-if="stats.last_cache_found_at">
              <p class="text-sm font-medium text-gray-500">Dernière cache trouvée</p>
              <p class="text-sm text-gray-900">{{ formatDate(stats.last_cache_found_at) }}</p>
            </div>
            <div v-if="stats.last_activity_at">
              <p class="text-sm font-medium text-gray-500">Dernière activité</p>
              <p class="text-sm text-gray-900">{{ formatDate(stats.last_activity_at) }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div class="space-y-4">
            ...
            <div v-if="stats.last_cache_found_at">
              <p class="text-sm font-medium text-gray-500">Jours depuis dernière cache</p>
              <p class="text-2xl font-semibold text-gray-900">{{ daysSinceLastCache }}</p>
            </div>

            <!-- Moyenne caches par challenge actif -->
            <div v-if="stats.active_challenges > 0">
              <p class="text-sm font-medium text-gray-500">Caches par challenge actif</p>
              <p class="text-2xl font-semibold text-gray-900">{{ cachesPerActiveChallenge }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="stats.total_caches_found === 0" class="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div class="flex">
          ...
          <div class="ml-3">
            <h3 class="text-sm font-medium text-blue-800">Prêt à commencer ?</h3>
