  ...
  <section class="max-w-screen-md mx-auto space-y-6">
    ...
    <form class="space-y-5" @submit.prevent="submit">
      ...
      <div class="space-y-1">
        ...
        <div
          class="grid grid-cols-[auto_1fr_auto_1fr] gap-x-3 gap-y-2 items-center"
        >
          <input
            id="mode-all"
            v-model="importMode"
  ...
  try {
    ...
    if (isAxiosError(e) && e.response) {
      ...
      if (status === 400) {
        ...
        error.value =
          detail ?? (status ? `Erreur ${status}` : "Import impossible.");
      }
    } else if (
      isAxiosError(e) &&
      (e.code === "ERR_NETWORK" || e.code === "ECONNRESET")
    ) {
